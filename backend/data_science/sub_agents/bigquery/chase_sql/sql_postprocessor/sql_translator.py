...
def _isinstance_bird_sample_type(obj: Any) -> bool:
    """Checks if the object is a Bird dataset sample."""
    # FIX: Strictly check for the presence of the 'db_table_names' key
    return isinstance(obj, dict) and "db_table_names" in obj


def _isinstance_bq_tool_schema_type(obj: Any) -> bool:
    """Checks if the object is the BQ tool schema format."""
    # Checks for the dict format: {table_name: {'table_schema': [...]}}
    if not isinstance(obj, dict):
        return False
    if not obj:
        return False
    # Check an arbitrary value to see if it matches the expected structure
    first_val = next(iter(obj.values()))
    return isinstance(first_val, dict) and "table_schema" in first_val
...
class SqlTranslator:
    ...
    @classmethod
    def _format_bq_tool_schema(cls, schema: dict) -> SQLGlotSchemaType:
        """Formats the BQ tool schema for SQLGlot."""
        # Convert {tbl: {'table_schema': [(col, type)...]}} to [(tbl, [(col, type)...])]
        # Then use format_schema logic.
        ddl_style_list = []
        for table_name, details in schema.items():
            columns = details.get("table_schema", [])
            ddl_style_list.append((table_name, columns))
        return cls.format_schema(ddl_style_list)
    ...
    def rewrite_schema_for_sqlglot(
        ...
        if schema:
            if isinstance(schema, str):
                ...
                schema_dict = schema
            # Check for BQ Tool Schema first or alongside others
            elif _isinstance_bq_tool_schema_type(schema):
                schema_dict = cls._format_bq_tool_schema(schema)
                ...
                # Fallback or TypeError. For now, we assume it might be a partial dict
                # or handled elsewhere, but raising TypeError is safer for debugging.
                raise TypeError(f"Unsupported schema type: {type(schema)} - {schema}")
        return schema_dict
